...
# --- AIによる市場特性・ターゲット分析ロジック ---
                    ...
                    for station_name in result_df.columns:
                        # 1. データの抽出（その駅の物件群）
                        # ※all_station_dataリストから該当駅のDataFrameを特定
                        st_df = next(item['data'] for item in all_station_data if item['station'] == station_name)
                        
                        st_mean = st_df['賃料(円)'].mean()
                        st_area_mean = st_df['専有面積(㎡)'].mean()
                        st_max = st_df['賃料(円)'].max()
                        
                        # --- 判定アルゴリズム：5つの性格とターゲット層を動的に生成 ---
                        
                        # A. ターゲット層の判定
                        if st_area_mean > 45:
                            target = "【ファミリー・DINKs層中心】"
                            t_desc = "広い専有面積の物件が主流で、居住環境を重視する世帯に人気です。"
                        elif st_area_mean < 25:
                            target = "【単身・学生層中心】"
                            t_desc = "効率的なワンルーム・1Kが市場の大部分を占める、利便性重視のエリアです。"
                        else:
                            target = "【単身〜カップル層混在】"
                            t_desc = "幅広い間取りが存在し、多様なライフスタイルに対応可能な市場です。"

                        # B. 市場の性格判定
                        if st_mean > all_stations_mean * 1.2:
                            personality = "ハイエンド市場。高額な賃料に見合うブランド力や設備を備えた物件が選好されます。"
                        elif st_mean < all_stations_mean * 0.8:
                            personality = "バリュー市場。手頃な家賃設定が多く、コストパフォーマンスが最大の魅力です。"
                        else:
                            personality = "バランス市場。相場が安定しており、需要と供給の均衡が取れたエリアです。"

                        # C. 統計的特徴（外れ値の有無など）
                        extra_comment = ""
                        if st_max > st_mean * 2.5:
                            extra_comment = " また、一部に相場を大きく超える超高級物件が供給されています。"

                        # 最終的なコメントの組み立て
                        full_comment = f"{target}\n{t_desc}\n{personality}{extra_comment}"
                        summary_data.append([station_name, full_comment])

                    # --- パワポに「AI詳細市場分析」スライドを追加 ---
                    slide_ai = ppt.slides.add_slide(ppt.slide_layouts[5])
                    slide_ai.shapes.title.text = "AIデータ解析：各駅のターゲット層と市場性格"

                    rows, cols = len(summary_data) + 1, 2
                    table_ai = slide_ai.shapes.add_table(rows, cols, Inches(0.5), Inches(1.5), Inches(9), Inches(5.5)).table

                    table_ai.cell(0, 0).text = "駅名"
                    table_ai.cell(0, 1).text = "AIによる市場分析（ターゲットと推奨層）"

                    for i, (name, comment) in enumerate(summary_data, start=1):
                        table_ai.cell(i, 0).text = name
                        # 改行を含む文章を挿入
                        cell_c = table_ai.cell(i, 1)
                        cell_c.text = comment
                        
                        # --- 文字の書式調整（少し細かく調整） ---
                        table_ai.cell(i, 0).text_frame.paragraphs[0].font.size = Pt(14)
                        table_ai.cell(i, 0).text_frame.paragraphs[0].font.bold = True
                        
                        # コメント欄の行間やサイズを調整
                        p_idx = 0
                        for paragraph in cell_c.text_frame.paragraphs:
                            paragraph.font.size = Pt(11)
                            # 1行目（ターゲット層）だけ太字にする
                            if p_idx == 0:
                                paragraph.font.bold = True
                                paragraph.font.color.rgb = RGBColor(0, 102, 204) # 青色
                            p_idx += 1
